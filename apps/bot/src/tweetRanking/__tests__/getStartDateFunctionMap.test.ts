...
])('testing dates', async ({ date, expected }: { date: Date, expected: Record<number, Function> }) => {
  const startDateMap: Record<number, Function> = getStartDateFunctionMap(getDayBeforeDate(date));

  expect(startDateMap).toEqual(expected);
});